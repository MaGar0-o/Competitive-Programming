...
void update(int id, int l, int r, int idx, int cnt){
	...
	update(id<<1|1, mid, r, idx, cnt);
	return;
}

LL get(int id, int l, int r, int val){
	if(siz[id] <= val) return sum[id];
	if(r - l == 1) return val * v[l];
	int mid = (l + r) >> 1;
	if(siz[id<<1|1] >= val) return get(id<<1|1, mid, r, val);
...
LL solve(int l, int r, int st, int en){
	...
	for(int i = mid; i <= r; i++) update(1, 0, m, b[i], -1);
	for(int i = st; i <= opt - 1; i++) update(1, 0, n, b[i], +1);
...
int32_t main(){
	...
	LL ans = 0;
	ans = max(a[s], solve(max(0LL, s - d + 1), s - 1, s, n - 1));
	reverse(b, b + n);
	reverse(a, a + n);
	s = n - 1 - s;
	smax(ans, max(a[s], solve(max(0LL, s - d + 1), s - 1, s, n - 1)));
	cout << ans << endl;
	return 0;
}
